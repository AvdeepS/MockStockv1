...
def record_user_metrics():
    ...
    cash_balance = int(st.session_state.cash)  # Convert to standard Python int
    total_pnl = int(calculate_total_pnl(user_id))  # Convert to standard Python int
    latest_round = int(st.session_state.round)  # Convert to standard Python int
    ...
    response = conn.client.from_("user_metrics").upsert([user_metrics_data]).execute()
...
def calculate_cash_from_orders(user_id):
    ...
    if orders_response.data:
        for order in orders_response.data:
            ...
            total_value = order['lots'] * order['price'] * 10000  # Convert lots to units
            stt = order['s31_fees']
            
            if order_type in ['Buy', 'Short Sell']:
                starting_cash -= (total_value + stt)
            elif order_type in ['Sell', 'Cover Short']:
                starting_cash += (total_value - stt)
...
def initialize_session():
        ...
        if st.session_state.user_id:
            st.session_state.cash = calculate_cash_from_orders(st.session_state.user_id)
        else:
        ...
        if 'Gold' not in st.session_state:
            st.session_state.commodities = {f'Round {i}': {} for i in range(1, 8)}
        if 'investment_banks' not in st.session_state:
            st.session_state.investment_banks = {}  # Investment Banks purchased
                ...
                for i in range(1, 8):
                    ...
                    if price is not None:
                        if security_type == 'stock':
                            ...
                            st.session_state.commodities[round_key][ticker] = price
...
def get_current_price(ticker):
    ...
    if ticker in st.session_state.commodities[current_round]:
        return st.session_state.commodities[current_round][ticker]
...
def record_transaction(order_type, ticker, quantity, price, stt, asset_type='stock'):
    ...
    # Calculate the total cost or proceeds of the transaction
    total_amount = quantity * price * 10000  # Convert lots to units
    total_cost = total_amount + stt

    if asset_type != 'forex':
        # Retrieve existing orders for this ticker and user
        orders_response = conn.client.from_("orders").select("*").eq("user_id", user_id).eq("ticker", ticker).execute()
        existing_orders = orders_response.data if orders_response.data else []

        # Build a new order dict based on the parameters (note: quantity is in lots)
        new_order = {
            "user_id": user_id,
            "team_name": team_name,
            "type": order_type,
        ...
        # Combine all orders (existing plus the new one)
        all_orders = existing_orders + [new_order]

        # Simulate the position calculation for this ticker
        simulated = {
            'long_quantity': 0,      # in units
            'long_total_cost': 0.0,  # total cost of long positions
            'short_quantity': 0,     # in units
            'short_total_value': 0.0 # total value of short positions
        }
        ...
        for order in all_orders:
            ...
            if ord_type == "Buy":
                if simulated['short_quantity'] > 0:
                    cover_qty = min(q, simulated['short_quantity'])
                    avg_short_price = (simulated['short_total_value'] / simulated['short_quantity']
                                       if simulated['short_quantity'] > 0 else 0)
                    simulated['short_quantity'] -= cover_qty
                    simulated['short_total_value'] -= avg_short_price * cover_qty
                    remaining_qty = q - cover_qty
                    if remaining_qty > 0:
                        simulated['long_quantity'] += remaining_qty
                        simulated['long_total_cost'] += remaining_qty * p
                else:
                    simulated['long_quantity'] += q
                    simulated['long_total_cost'] += q * p

            elif ord_type == "Sell":
                if simulated['long_quantity'] >= q:
                    avg_long_price = (simulated['long_total_cost'] / simulated['long_quantity']
                                      if simulated['long_quantity'] > 0 else p)
                    simulated['long_quantity'] -= q
                    simulated['long_total_cost'] -= avg_long_price * q
                else:
                    extra_short_qty = q - simulated['long_quantity']
                    avg_long_price = (simulated['long_total_cost'] / simulated['long_quantity']
                                      if simulated['long_quantity'] > 0 else p)
                    simulated['long_quantity'] = 0
                    simulated['long_total_cost'] = 0.0
                    simulated['short_quantity'] += extra_short_qty
                    simulated['short_total_value'] += extra_short_qty * p

            elif ord_type == "Short Sell":
                simulated['short_quantity'] += q
                simulated['short_total_value'] += q * p

            elif ord_type == "Cover Short":
                if simulated['short_quantity'] >= q:
                    avg_short_price = (simulated['short_total_value'] / simulated['short_quantity']
                                       if simulated['short_quantity'] > 0 else p)
                    simulated['short_quantity'] -= q
                    simulated['short_total_value'] -= avg_short_price * q
                else:
                    extra_long_qty = q - simulated['short_quantity']
                    simulated['short_quantity'] = 0
                    simulated['short_total_value'] = 0.0
                    simulated['long_quantity'] += extra_long_qty
                    simulated['long_total_cost'] += extra_long_qty * p

        # Enforce cap: reject order if the resulting original investment exceeds $1B
        if order_type in ["Buy", "Short Sell"]:  # Cap applies only to new investments
            if order_type == "Buy" and (simulated["long_total_cost"] > 1e9):
                st.error(f"Transaction would exceed $1B cap on original long investments for {ticker}.")
                return

            if order_type == "Short Sell" and (simulated["short_total_value"] > 1e9):
                st.error(f"Transaction would exceed $1B cap on original short investments for {ticker}.")
                return

        # Deduct or add cash based on the transaction type
        if order_type in ["Buy", "Short Sell"]:
            if st.session_state.cash < total_cost:
                st.error("Insufficient funds to complete the transaction.")
                return
            st.session_state.cash -= total_cost
        elif order_type in ["Sell", "Cover Short"]:
            st.session_state.cash += total_amount - stt

        # Record the transaction in the orders table after successful cap check and cash adjustment
        conn.client.from_("orders").insert([new_order]).execute()

    else:
        # Handle Forex transaction
        if order_type in ["Buy", "Short Sell"]:
            if st.session_state.cash < total_cost:
                st.error("Insufficient funds to complete the forex transaction.")
                return
            st.session_state.cash -= total_cost
        elif order_type in ["Sell", "Cover Short"]:
            st.session_state.cash += total_amount - stt

        # Record the forex transaction
    ...
    st.success(f"Transaction recorded successfully. New cash balance: ${st.session_state.cash:.2f}")
...
INVESTMENT_BANKS = {
    "Lemon Brothers": {
        "price": 478500000,
        "description": "Experts in navigating both booms and busts, Lemon Brothers have an uncanny ability to read the markets before the rest catch on. Whether it’s spotting hidden opportunities or warning of unseen risks, their insights keep investors ahead of the curve. Their research is top-tier, but tends to end with a lemon emoji for no apparent reason."
    },
    "Ali Bye-Bye": {
        "price": 420690000,
        "description": "Specializing in South Asian markets, Ali Bye-Bye promises to say 'bye-bye' to your financial troubles with high-quality insights on emerging economies and government policy shifts. Their analysts have an uncanny ability to predict regulatory changes before they happen, and their network of local insiders ensures you're always ahead of the curve."
    },
    "BlackSock": {
        "price": 495400000,
        "description": "A powerhouse in asset management and structured finance, BlackSock provides rock-solid tech and banking insights - with an unusual obsession with footwear. Their analysts insist that wearing mismatched socks improves risk-adjusted returns, and every report includes a bonus 'footnote'."
    },
    "Kela Investments": {
        "price": 440548000,
        "description": "With a motto that 'ripe opportunities never go to waste,' this firm specializes in consumer goods and financial markets. Their reports are packed with sharp insights, along with an oddly high number of banana metaphors."
    }
}
initialize_session()
    ...
    if st.button("Login"):
        try:
            ...
            if response.data and response.data["password"] == password:
                ...
                record_user_metrics()
                fetch_user_metrics(st.session_state.user_id)
...
# ---- If Logged In, Show Order Form ----
    ...
    def sell_shares(company, shares, current_price):
        ...
        if company not in st.session_state.portfolio:
            # Treat as short sell
            total_short_value = shares * current_price * 10000
            stt = calculate_stt('equity', total_short_value)
            total_short_value_after_stt = total_short_value - stt

            if total_short_value_after_stt > st.session_state.cash:
                st.error("Not enough cash to short sell these shares!")
                return

            # Only create short position if there's enough cash
            ...
            st.session_state.short_positions[company]['shares'] += shares
            st.session_state.short_positions[company]['short_price'] = current_price
            st.session_state.short_positions[company]['short_value'] += total_short_value

            record_transaction('Short Sell', company, shares, current_price, stt)
            st.success(f"✅ Successfully short sold {shares} shares of {company}! (STT: ${stt:.2f})")
            st.rerun()
    ...
    def sell_forex(currency, amount):
        ...
        if currency in st.session_state.forex and st.session_state.forex[currency]['amount'] > 0:
            ...
            # If the user wants to short sell more forex beyond selling the long position
            remaining_amount = amount - amount_to_sell
            if remaining_amount > 0:
                total_short_value = remaining_amount * exchange_rate * 10000
                stt = calculate_stt('forex', total_short_value)
                total_short_value_after_stt = total_short_value - stt

                if total_short_value_after_stt > st.session_state.cash:
                    st.error("Not enough cash to short sell this amount of forex!")
                    return
                ...
                st.session_state.short_positions[currency]['short_value'] += total_short_value
            ...
            total_short_value = amount * exchange_rate * 10000
            stt = calculate_stt('forex', total_short_value)
            total_short_value_after_stt = total_short_value - stt
    ...
    def sell_derivative(company, contracts):
        ...
        if company in st.session_state.derivatives:
            ...
            if derivative_data['contracts'] >= contracts:
                ...
                stt_short = calculate_stt('derivatives', short_value_extra)
                total_short_value_after_stt = short_value_extra - stt_short

                if total_short_value_after_stt > st.session_state.cash:
                    st.error("Not enough cash to short sell the remaining contracts!")
                    return
                ...
                record_transaction('Short Sell', company, contracts_to_short, current_price, stt_short)
            ...
            if total_value > st.session_state.cash:
                st.error("Not enough cash to short sell these contracts!")
                return
            ...
            st.session_state.short_positions[company]['short_value'] += total_value
    ...
    def buy_gold(quantity):
        if quantity <= 0:
            st.error("Please enter a valid quantity of gold to buy.")
            return

        current_price = get_current_price("Gold")
        if current_price is None:
            st.error("Gold price data not available for the current round.")
            return

        # Check if there's an open short position
        if "Gold" in st.session_state.short_positions:
            short_data = st.session_state.short_positions["Gold"]
            quantity_to_cover = min(quantity, short_data['quantity'])
            cover_value = quantity_to_cover * current_price * 10000
            stt = calculate_stt('commodity', cover_value)
            st.session_state.cash += cover_value - (3 * stt)
            st.success(f"💰 ${cover_value:.2f} added to your cash balance from covering {quantity_to_cover} shorted gold units!")
            short_data['quantity'] -= quantity_to_cover
            if short_data['quantity'] <= 0:
                del st.session_state.short_positions["Gold"]
            record_transaction('Cover Short', "Gold", quantity_to_cover, current_price, stt)
            quantity -= quantity_to_cover
            if quantity <= 0:
                st.rerun()

        # Normal buy flow for remaining quantity
        if quantity > 0:
            total_cost = quantity * current_price * 10000
            stt = calculate_stt('commodity', total_cost)
            total_cost_with_stt = total_cost + stt
            if total_cost_with_stt > st.session_state.cash:
                st.error("Not enough cash to buy this quantity of gold!")
                return
            if "Gold" not in st.session_state.commodities:
                st.session_state.commodities["Gold"] = {"quantity": 0, "total_spent": 0, "total_stt": 0}
            st.session_state.commodities["Gold"]["quantity"] += quantity
            st.session_state.commodities["Gold"]["total_spent"] += total_cost
            st.session_state.commodities["Gold"]["total_stt"] += stt
            record_transaction('Buy', "Gold", quantity, current_price, stt)
            st.success(f"✅ Successfully bought {quantity} units of gold at ${current_price:.2f} per unit!")
            st.rerun()


    def sell_gold(quantity):
        if quantity <= 0:
            st.error("Please enter a valid quantity of gold to sell.")
            return
        ...
        # Check if user has existing long positions
        if "Gold" in st.session_state.commodities and st.session_state.commodities["Gold"]["quantity"] > 0:
            gold_data = st.session_state.commodities["Gold"]
            quantity_to_sell = min(quantity, gold_data["quantity"])
            total_received = quantity_to_sell * current_price * 10000
            stt = calculate_stt('commodity', total_received)
            total_received_after_stt = total_received - stt

            gold_data["quantity"] -= quantity_to_sell
            gold_data["total_spent"] -= total_received
            gold_data["total_stt"] += stt

            record_transaction('Sell', "Gold", quantity_to_sell, current_price, stt)
            st.success(f"✅ Successfully sold {quantity_to_sell} units of gold at ${current_price:.2f} per unit!")

            quantity -= quantity_to_sell
            if quantity <= 0:
                st.rerun()

        # Short sell remaining quantity
        if quantity > 0:
            total_short_value = quantity * current_price * 10000
            stt = calculate_stt('commodity', total_short_value)
            total_short_value_after_stt = total_short_value - stt

            if total_short_value_after_stt > st.session_state.cash:
                st.error("Not enough cash to short sell this quantity of gold!")
                return

            if "Gold" not in st.session_state.short_positions:
                st.session_state.short_positions["Gold"] = {"quantity": 0, "short_value": 0}
            st.session_state.short_positions["Gold"]["quantity"] += quantity
            st.session_state.short_positions["Gold"]["short_value"] += total_short_value

            record_transaction('Short Sell', "Gold", quantity, current_price, stt)
            st.success(f"✅ Successfully short sold {quantity} units of gold at ${current_price:.2f} per unit!")
            st.rerun()
    ...
    def buy_investment_bank(bank_name):
        if bank_name not in INVESTMENT_BANKS:
            st.error("Invalid Investment Bank selected.")
            return

        bank_details = INVESTMENT_BANKS[bank_name]
        price = bank_details["price"]

        if price > st.session_state.cash:
            st.error("Not enough cash to buy this Investment Bank!")
            return

        # Deduct the cost from cash
        st.session_state.cash -= price

        # Add the Investment Bank to the portfolio
        if "investment_banks" not in st.session_state:
            st.session_state.investment_banks = {}
        st.session_state.investment_banks[bank_name] = {"price": price, "description": bank_details["description"]}

        # Record the transaction
        st.session_state.transactions.append({
            'round': st.session_state.round,
            'type': 'Buy Investment Bank',
            'bank_name': bank_name,
            'price': price
        })

        st.success(f"✅ Successfully purchased {bank_name} for ${price:.2f}!")

    def display_investment_bank_reports():
        if "investment_banks" in st.session_state and st.session_state.investment_banks:
            st.subheader("📊 Investment Bank Reports")
            for bank_name, details in st.session_state.investment_banks.items():
                st.write(f"**{bank_name}**")
                st.write(f"**Price Paid:** ${details['price']:.2f}")
                st.write(f"**Description:** {details['description']}")
                st.write("---")
        else:
            st.write("No Investment Banks purchased yet.")
    ...
    st.sidebar.markdown(f"📈 Unrealised Profit/Loss ($): :{pnl_color}[{total_pnl:,.2f}]")
    ...
    round_securities = {
    1: ["Shares", "Forex","Commodity"],
    2: ["Shares", "Forex","Commodity"],
    3: ["Shares", "Forex", "Derivatives","Commodity"],
    4: ["Shares", "Forex", "Derivatives","Commodity", "Insiders","Investment Banks" ],
    5: ["Shares", "Forex", "Derivatives","Commodity", "Green Bonds"],
    6: ["Shares", "Forex", "Derivatives","Commodity", "Green Bonds"],
    7: ["Shares", "Forex", "Derivatives","Commodity", "Green Bonds"]
}

# Get the securities for the current round
    current_securities = round_securities.get(st.session_state.round, [])
    ...
    if selected_security == "Shares":
        ...
        shares = st.sidebar.number_input(f"Number of LOTS for {company_selected}", min_value=0)
    ...
    elif selected_security == "Investment Banks":
        if st.session_state.round == 4:
            st.subheader("🏦 Investment Banks (Round 4)")
            st.write("Purchase Investment Banks to receive exclusive market insights and reports.")

            # Display available Investment Banks
            st.write("### Available Investment Banks")
            for bank_name, details in INVESTMENT_BANKS.items():
                st.write(f"**{bank_name}**")
                st.write(f"**Price:** ${details['price']:.2f}")
                st.write(f"**Description:** {details['description']}")
                if st.button(f"Buy {bank_name}"):
                    buy_investment_bank(bank_name)
                st.write("---")

            # Display purchased Investment Banks and their reports
            display_investment_bank_reports()
        else:
            st.info("Investment Banks are only available in Round 4.")
    ...
    elif selected_security == "Insiders":
        if st.session_state.round == 4:
    ...
    elif selected_security == "Commodity":
        st.subheader("🥇 Commodity Trading (Gold)")
        st.write(f"Current Gold Price (Round {st.session_state.round}): ${GOLD_PRICES[st.session_state.round]:.2f}")

        quantity = st.number_input("Quantity of Gold", min_value=0)
    ...
    if st.sidebar.button("Submit Round"):
        if not confirmation:
            ...
            if st.session_state.round < 7:
                # Record metrics for the current round before incrementing
                record_user_metrics()
                st.session_state.round += 1
