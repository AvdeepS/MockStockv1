...
# ---- If Logged In, Show Order Form ----
    ...
    def process_futures_expiry():
        ...
        for key, data in list(st.session_state.derivatives.items()):
            parts = key.split("_")
            if len(parts) == 3 and parts[1] == "Fut":  # Format: CompanyName_Fut_R#
                company = parts[0]
                ...
                if expiry_round == current_round:
                    ...
                    # Calculate PNL
                    pnl = (settlement_price - original_price) * contracts * contract_size

                    # Deduct remaining 75% margin
                    remaining_margin = contracts * original_price * contract_size * 0.75

                    # Adjust cash balance
                    st.session_state.cash += pnl - remaining_margin

                    # Remove the expired future from derivatives
                    del st.session_state.derivatives[key]

                    # Record transaction for settlement
                    record_transaction('Settle Future', company, contracts, settlement_price, 0)

                    expired_futures.append(company)

        # Handle short positions
        for company, short_data in list(st.session_state.short_positions.items()):
            if 'expiry_round' in short_data and short_data['expiry_round'] == current_round:
                round_key = f"Round {current_round}"
                future_data = st.session_state.derivatives_data.get(round_key, {}).get("Future", {}).get(f"{company}_Fut_R{current_round}")
                
                if not future_data or 'price' not in future_data:
                    st.error(f"Price data missing for {company} short position in Round {current_round}.")
                    continue

                settlement_price = future_data['price']
                contract_size = future_data.get('contract_size', 10000)
                original_price = short_data['short_price']
                contracts = short_data['contracts']

                # Calculate PNL for short position
                pnl = (original_price - settlement_price) * contracts * contract_size
                ...
                # Remove the expired short position
                del st.session_state.short_positions[company]

                # Record transaction for settlement of short position
                record_transaction('Settle Short Future', company, contracts, settlement_price, 0)

                expired_futures.append(f"{company} (Short)")
