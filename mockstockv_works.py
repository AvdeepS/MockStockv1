...
def initialize_session():
        ...
        if 'commodities' not in st.session_state:
            st.session_state.commodities = {}  # Commodities (e.g., Gold) holdings
...
def record_transaction(order_type, ticker, quantity, price, stt, asset_type='stock'):
    ...
    if asset_type != 'forex':
        ...
        for order in all_orders:
            ord_type = order['type']
            # Convert lots to actual units (each lot equals 10,000 units)
            q = order['lots'] * 10000
            p = order['price']
            ...
            if ord_type == "Buy":
                ...
                if simulated['long_quantity'] >= q:
                    ...
                    # If there are long holdings, use its average; otherwise use current price
                    avg_long_price = (simulated['long_total_cost'] / simulated['long_quantity']
                                      if simulated['long_quantity'] > 0 else p)
        ...
        # Record the transaction in the orders table after successful cap check
        conn.client.from_("orders").insert([new_order]).execute()

    else:
        # Handle Forex transaction directly
...
# ---- If Logged In, Show Order Form ----
    ...
    def buy_gold(quantity):
        ...
        current_price = GOLD_PRICES[st.session_state.round]
    ...
    def sell_gold(quantity):
        ...
        if "Gold" not in st.session_state.commodities or st.session_state.commodities["Gold"]["quantity"] < quantity:
            st.error("You don't own enough gold to sell!")
            return
        ...
        st.session_state.commodities["Gold"]["quantity"] -= quantity
        st.session_state.commodities["Gold"]["total_spent"] -= total_received
        st.session_state.commodities["Gold"]["total_stt"] += stt
        st.session_state.cash += total_received_after_stt

        if st.session_state.commodities["Gold"]["quantity"] <= 0:
            del st.session_state.commodities["Gold"]
        ...
        st.success(f"✅ Successfully sold {quantity} units of gold at ${current_price:.2f} per unit!")
        st.rerun()
    ...
    selected_security = st.sidebar.selectbox(
        ...
        ["Shares", "Forex", "Derivatives", "Green Bonds", "Insider","Investment Banks", "Commodity"]
    )
    ...
    # Derivatives Trading Section
        ...
        if "Gold" in st.session_state.commodities:
            st.subheader("🥇 Gold Holdings")
            gold_data = st.session_state.commodities["Gold"]
            st.write(f"**Quantity:** {gold_data['quantity']}")
            st.write(f"**Total Spent:** ${gold_data['total_spent']:.2f}")
            st.write(f"**Current Value:** ${gold_data['quantity'] * GOLD_PRICES[st.session_state.round]:.2f}")
